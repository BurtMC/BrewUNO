    ...
    if (configFile)
    {
        ...
        if (size <= MAX_ACTIVESTATUS_SIZE)
        {
            ...
            if (error == DeserializationError::Ok && _activeStatusJsonDocument.is<JsonObject>())
            {
                ...
                Rotation= _activeStatus["rotation"];
...
String ActiveStatus::GetJson()
{
    ...
    active_status.replace("{{AgitatorOn}}", String(AgitatorOn));
    active_status.replace("{{AgitatorIsResting}}", String(AgitatorIsResting));
...
void ActiveStatus::SaveActiveStatus()
{
    ...
    _activeStatus["rotation"] = Rotation;
