#include <MashService.h>
  ...
  MashService::MashService(FS *fs, TemperatureService *temperatureService, 
                            #if defined(PUMP_BUS) 
                                Pump *pump
...
void MashService::NextStep(ActiveStatus *activeStatus, JsonArray steps, time_t timeNow, int nextMashStep)
{
    ...
    activeStatus->Rotation = ((int)step["r"]) == 1;
...
void MashService::BoilTime(ActiveStatus *activeStatus)
{
    ...
    Buzzer().Ring(2, 2000);
        ...
        _pump->TurnPumpOff();
    #elif defined(AGITATOR_BUS)
        activeStatus->Rotation = false;
        _agitator->TurnAgitatorOff();
    #endif
    activeStatus->SaveActiveStatus();
}
...
void MashService::StepStarted(ActiveStatus *activeStatus, JsonArray steps, time_t timeNow)
{
    ...
    activeStatus->Recirculation = ((int)step["r"]) == 1;
    if (activeStatus->Recirculation || activeStatus->Rotation)
        #if defined(PUMP_BUS)
            _pump->TurnPumpOn();
        #elif defined(AGITATOR_BUS)
            _agitator->TurnAgitatorOn();
